...
export default function LoginScreen({ route }) {
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        Alert.alert(t.signupSuccess, t.signupPending);
        setIsLogin(true);
      }
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>{t.customerTitle}</Text>
      ...
      <TouchableOpacity style={styles.button} onPress={handleAuth} disabled={loading}>
        {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>{isLogin ? t.login : t.signup}</Text>}
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 32, fontWeight: 'bold', marginBottom: 5, textAlign: 'center', color: '#007AFF' },
  ...
  button: { backgroundColor: '#007AFF', padding: 16, borderRadius: 10, alignItems: 'center', elevation: 2 },
